...
export class TripLifecycleService {
    private static validTransitions: Record<string, string[]> = {
        [TripStatus.WAITING_DRIVER]: [TripStatus.DRIVER_ARRIVED, TripStatus.CANCELLED],
     ...
     */
    static async changeStatus(tripId: string, userId: string, nextStatus: TripStatus, options: { role?: "driver" | "passenger", extraData?: any } = {}): Promise<AsyncCustomResponse> {
        try {
            ...
            if (!this.isValidTransition(trip.status, nextStatus)) {
                throw new ErrorMsg(`Transition from ${trip.status} to ${nextStatus} is not allowed`, 400);
            }
            ...
            if (nextStatus === TripStatus.COMPLETED) {
                ...
                const updatedWallet = await WalletModel.findByIdAndUpdate(trip.driver.wallet._id, { balance: driverBalance }, { new: true }).exec();
                trip.final_fare = trip.final_fare ?? trip.total_fare;
                trip.status = TripStatus.COMPLETED;
                await trip.save();

                // notificar
                await NotificationService.notifyTripChange(trip, "TRIP_FINISHED", { commission, driverEarnings });

                return {
                    message: "Trip finished successfully",
                    info: { newTripStatus: trip.status, commission, driverEarnings, driverWallet: updatedWallet }
                };
            }
            ...
            if (event) {
                await NotificationService.notifyTripChange(trip, event, options.extraData);
            }
    ...
    static async markDriverArrived(tripId: string, driverId: string) {
        return this.changeStatus(tripId, driverId, TripStatus.DRIVER_ARRIVED, { role: "driver" });
    }
    static async startTrip(tripId: string, driverId: string) {
        return this.changeStatus(tripId, driverId, TripStatus.IN_PROGRESS, { role: "driver" });
    }
    static async finishTrip(tripId: string, driverId: string) {
        return this.changeStatus(tripId, driverId, TripStatus.COMPLETED, { role: "driver" });
    }
    static async cancelTrip(tripId: string, userId: string) {
        return this.changeStatus(tripId, userId, TripStatus.CANCELLED, { role: "passenger" });
    }
}
